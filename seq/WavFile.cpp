...
//const unsigned int WavFile::samplesPerSecond = 48000;

void WavFile::addSound(const Sound & sound) {
	this->sound = &sound;
}

void WavFile::generate() {
	sound->init();
	...
	for (unsigned int t = 0; t < numSamples; ++t) {
		...
		signed short s = (signed short) (rampMultiplier * (SHRT_MAX >> 1) * sound->sample(time));
